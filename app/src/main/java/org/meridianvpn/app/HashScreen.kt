 ...
 *   3. обычная ссылка в браузер — на vk.ru, а не vk.com.
 *
 * ПОЧЕМУ vk.ru, А НЕ vk.com (17.09, просьба владельца). Приложение VK
 * убрали из Google Play — часть людей физически не смогут поставить
 * его и попадут ровно в эту, третью ветку. Раньше она вела на vk.com;
 * проверено вручную (17.09): vk.ru отвечает и остаётся действующим
 * сайтом ВК, тогда как vk.com для человека без приложения — путь с
 * меньшей вероятностью открыться нормально. Схема (шаг 1) и имена
 * пакетов (шаг 2) не меняем: это внутренние идентификаторы самого
 * приложения VK, к домену сайта отношения не имеют.
 *
...
private fun openVk(ctx: Context) {
    ...
    try {
        ctx.startActivity(
            Intent(Intent.ACTION_VIEW, Uri.parse("https://vk.ru/im"))
                .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        )
    } catch (e: Throwable) {
