...
// быстрая на старте.
//
// 0,8 с, было 1,5 (решение владельца 25.09, ускорение). Рукопожатие DTLS,
// которое проходит с первой попытки, встаёт за 0,3–0,4 с (замеры 24.09 на
// MTS: D за 355 мс, B за 320 мс), и 0,8 с накрывают его с двойным запасом.
// Не накрывают только рукопожатие, потерявшее первый пакет (повтор pion на
// 1 с) — такое и раньше едва укладывалось. Выигрыш — 0,7 с на каждой
// гонке, где DTLS не отвечает вовсе (MTS 24.09). Цена — ступень без DTLS
// чаще побеждает медленный DTLS; это ловят метки и память пути.
const noDtlsHandicap = 800 * time.Millisecond
